...
function GiveData($loginStud)
{
   ...
   $result = mssql_query("SELECT A.idStud, CONCAT(B.Name_F,' ',B.Name_I,' ',B.Name_O), B.Idf, B.IdKurs, B.IdGroup, C.Name, LEFT(C.Name,1), D.Name FROM dbo.Student_logins_2 A LEFT JOIN dbo.Student B ON B.IdStud=A.idStud LEFT JOIN dbo.Groups C ON C.IdGroup=B.IdGroup LEFT JOIN dbo.Facultets D ON D.Idf=B.Idf WHERE A.login='".$_SERVER['REMOTE_USER']."'",$dbStud);
