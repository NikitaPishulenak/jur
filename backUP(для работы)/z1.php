...
function GroupViewL(){
    ...
    $retVal.="������ � ".$_GET['nGroup']." (<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".
        $_SESSION['SesVar']['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF']."&idGroup=".$_GET
        ['idGroup']."&PL=0&nPredmet=".$_GET['nPredmet']."&nF=".$_GET['nF']."&nGroup=".$_GET['nGroup']."'>������������</a> / ������)</h3><hr>";
    ...
    $result = mssql_query("SELECT IdStud, CONCAT(Name_F,' ',Name_I,' ',Name_O) FROM dbo.Student WHERE IdGroup=".$_GET['idGroup']." 
AND IdStatus IS NULL ORDER BY Name_F",$dbStud);
    $resultL = mysqli_query($dbMain, "SELECT id, LDate, PKE, DATE_FORMAT(LDate,'%e.%m.%Y') FROM lesson WHERE idGroup=".$_GET
        ['idGroup']." AND idLessons=".$_GET['idPredmet']." AND PL=1 ORDER BY LDate,id");
        ...
        if(mssql_num_rows($result)>=1){
            ...
            if(mysqli_num_rows($resultL)>=1){
                while($arr = mysqli_fetch_row($resultL)){
                    ...
                    switch($arr[2]){
                        case 1:
                            $prepreRating.="<div class='date_col colloquium_theme'><div class='date_title' data-idLesson='".$arr
                                [0]."'>".$arr[3]."</div>\n";
                            break;
                        case 2:
                            $prepreRating.="<div class='date_col exam_theme'><div class='date_title' data-idLesson='".$arr[0]."'>".$arr
                                [3]."</div>\n";
                            break;
                        default:
...
\n";
                    ...
                    $resultS = mysqli_query($dbMain, "SELECT id, idStud, RatingO FROM rating WHERE (".$sqlStud.") AND PKE=".$arr[2]." 
AND idLesson=".$arr[0]." AND idLessons=".$_GET['idPredmet']." AND PL=1");
                    $arrSStud = Array();
                    ...
                    for($iS=0; $iS<=($countarrStud-1); $iS++){
                        ...
                        for($iSS=0; $iSS<=($countSStud-1); $iSS++){
                            if($arrStud[$iS]==$arrSStud[$iSS][1]){
                                $prepreRating.="<div class='grade' data-idLes=".$arr[0]." data-idStudent=".$arrStud[$iS]." data-PKE=".
                                    $arr[2]." data-zapis=".$arrSStud[$iSS][0].">".$arrSStud[$iSS][2]."</div>\n";
                                $trueS=1;
                            }
                        }
                        if(!$trueS){ $prepreRating.="<div class='grade' data-idLes=".$arr[0]." data-idStudent=".$arrStud[$iS]." data-
PKE=".$arr[2]." data-zapis=0></div>\n"; }
                    }
...
function edtLessonStudent(){
    include_once 'configMain.php';
    mysqli_query($dbMain, "UPDATE rating SET RatingO=".$_GET['grades']." WHERE id=".$_GET['id_Zapis']." AND idStud=".$_GET
        ['idStudent']);
}
...
function addLessonStudent(){
    ...
    mysqli_query($dbMain, "INSERT INTO rating (idLessons,idStud,PL,PKE,DateO,TimeO,RatingO,idEmployess,idLesson) VALUES (".$_GET
        ['idLessons'].",".$_GET['idStudent'].",".$_GET['PL'].",".$_GET['PKE'].",'".$dt[2]."-".$dt[1]."-".$dt[0]."',CURTIME(),".$_GET
        ['grades'].",".$_SESSION['SesVar']['FIO'][0].",".$_GET['idLess'].")");
...
//   $result = mysqli_query($dbMain, "SELECT id FROM lesson WHERE LDate='".$dt[2]."-".$dt[1]."-".$dt[0]."' AND idGroup=".$_GET
    ['idGroup']." AND idLessons=".$_GET['idLessons']." AND PL=".$_GET['PL']." AND PKE=".$_GET['PKE']);
//   if(mysqli_num_rows($result)>=1){
//      echo "No";
//   } else {
      mysqli_query($dbMain, "INSERT INTO lesson (LDate,idGroup,idLessons,PL,PKE) VALUES ('".$dt[2]."-".$dt[1]."-".$dt[0]."',".
          $_GET['idGroup'].",".$_GET['idLessons'].",".$_GET['PL'].",".$_GET['PKE'].")");
      echo mysqli_insert_id($dbMain);
//   }
...
function GroupViewP(){
    ...
    $retVal.="<h3>".$_GET['nPredmet']."<br>&nbsp;<font color='#ff0000'>&darr;</font><br>";
    $retVal.=$_GET['nF']."<br>&nbsp;<font color='#ff0000'>&darr;</font><br>";
    $retVal.="������ � ".$_GET['nGroup']." (������������ / <a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO']
        [0]."&idKaf=".$_SESSION['SesVar']['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF']."&idGroup=".$_GET
        ['idGroup']."&PL=1&nPredmet=".$_GET['nPredmet']."&nF=".$_GET['nF']."&nGroup=".$_GET['nGroup']."'>������</a>)</h3><hr>";
...
AND IdStatus IS NULL ORDER BY Name_F",$dbStud);
    $resultL = mysqli_query($dbMain, "SELECT id, LDate, PKE, DATE_FORMAT(LDate,'%e.%m.%Y') FROM lesson WHERE idGroup=".$_GET
        ['idGroup']." AND idLessons=".$_GET['idPredmet']." AND PL=0 ORDER BY LDate,id");
...
\n";
                    ...
                    $resultS = mysqli_query($dbMain, "SELECT id, idStud, RatingO FROM rating WHERE (".$sqlStud.") AND PKE=".$arr[2]." 
AND idLesson=".$arr[0]." AND idLessons=".$_GET['idPredmet']." AND PL=0");
                    $arrSStud = Array();
                    if(mysqli_num_rows($resultS)>=1){
                        $ii=0;
                        while($arrSS = mysqli_fetch_row($resultS)){
...
function Fakultet(){
    ...
    if(mysqli_num_rows($resPredmet)>=1){
      ...
      ";
            if($_GET['idF']==283){
                $result = mssql_query("SELECT IdGroup, Name FROM dbo.Groups WHERE ((IdF=".$_GET['idF']." AND DATEDIFF(month,CONCAT
(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) OR (LEFT(Name,1)='".$fac[$_GET['idF']][0]."' AND DATEDIFF
(month,CONCAT(Year,'0101'),GETDATE())<=".$fac[$_GET['idF']][1]." AND LEN(Name)>=4)) OR ((IdF=".$_GET['idF']." AND DATEDIFF
(month,CONCAT(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][3]." AND LEN(Name)>=4) OR (LEFT(Name,1)='".$fac[$_GET['idF']][2]."' AND 
DATEDIFF(month,CONCAT(Year,'0101'),GETDATE())<=".$fac[$_GET['idF']][3]." AND LEN(Name)>=4)) ORDER BY Name",$dbStud);
            } else {
                $result = mssql_query("SELECT IdGroup, Name FROM dbo.Groups WHERE (IdF=".$_GET['idF']." AND DATEDIFF(month,CONCAT
(Year,'0101'),GETDATE())<".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) OR (LEFT(Name,1)='".$fac[$_GET['idF']][0]."' AND DATEDIFF
(month,CONCAT(Year,'0101'),GETDATE())<=".$fac[$_GET['idF']][1]." AND LEN(Name)>=4) ORDER BY Name",$dbStud);
            }
            if(mssql_num_rows($result)>=1){
                $preChar="";
                while($arr = mssql_fetch_row($result)){
                    ...
                    $retVal.="<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']
                        ['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF']."&idGroup=".$arr[0]."&PL=0&nPredmet=".$Pre."&nF=".
                        $idName."&nGroup=".$arr[1]."'>�����.</a>&nbsp;&nbsp;&nbsp;";
                    $retVal.="<a href='z.php?menuactiv=goG&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']
                        ['Zav'][2]."&idPredmet=".$_GET['idPredmet']."&idF=".$_GET['idF']."&idGroup=".$arr[0]."&PL=1&nPredmet=".$Pre."&nF=".
                        $idName."&nGroup=".$arr[1]."'>������</a>";
                    $retVal.="</div></div></div>\n";
                }
            }
   ...
   ";
    ...
    for($ii=0; $ii<=($countPredmet-1); $ii++){
        ...
        for($i=0; $i<=($countFak-1); $i++){
            ...
            if(mssql_num_rows($result)>=1){
                list($idName)=mssql_fetch_row($result);
                $retVal.="<p><a href='z.php?menuactiv=goF&idPrepod=".$_SESSION['SesVar']['FIO'][0]."&idKaf=".$_SESSION['SesVar']
                    ['Zav'][2]."&idPredmet=".$_SESSION['SesVar']['PredmetZav'][$ii][0]."&idF=".$fak[$i]."'>".$idName."</a></p>";
            }
        }
        $retVal.="</div>";
    }
...
placeholder='дд.мм.гггг'>
                ...
                <br>
                <label><input type='radio' class='type_lesson' id='simple_lesson_rb' name='type_lesson' value='sl' checked><b 
class='type_lesson'>Обычное занятие</b></label>
                <br><br>
                <label><input type='radio' class='type_lesson' id='colloquium_rb' name='type_lesson' value='col'><b 
class='type_lesson'>Коллоквиум</b></label>
                <br><br>
                <label><input type='radio' class='type_lesson' id='exam_rb' name='type_lesson' value='exam'><b 
class='type_lesson'>Аттестация</b></label>
                <br><br>
            </div>
        </fieldset>
    </form>
...
<div id='form-edit' title='Редактировать отметку'>
    <form id='form-edit'>
        <fieldset>
            <div class='panel'>
                <button id='add_grade_input' class='add_grade' title='Для добавления дополнительной оценки нажмите на кнопку!'>
+</button>
...
<div id=\"form-edit-date\" title=\"Редактирование даты занятия\">
    <form>
        <fieldset>
            <div class=\"box\">
                <b align='center'>Дата занятия</b>
                    <input type='text' id='edit-lesson-date' required class='datepicker' value='".date('d.m.Y')."' 
placeholder='дд.мм.гггг'>
                <br><br>
                <label><input type='radio' class='edit_type_lesson' id='edit_simple_lesson_rb' name='type_lesson' value='0' 
checked><b class='type_lesson'>Обычное занятие</b></label>
                <br><br>
                <label><input type='radio' class='edit_type_lesson' id='edit_colloquium_rb' name='type_lesson' value='1'><b 
class='type_lesson'>Коллоквиум</b></label>
                <br><br>
                <label><input type='radio' class='edit_type_lesson' id='edit_exam_rb' name='type_lesson' value='2'><b 
class='type_lesson'>Аттестация</b></label>
                <br>
            </div>
        </fieldset>
    ...
    if($countLev>=2){
        ...
        for($ii=0; $ii<=($countLev-1); $ii++){
            switch($_SESSION['SesVar']['Level'][$ii]){
                case 1:
                    $preDiv.="<p><a href='r.php'><strong>".$_SESSION['SesVar']['Rector'][0]."</strong> (".$_SESSION['SesVar']
                        ['Rector'][1].")</a></p>";
                    break;
                case 2:
                    $preDiv.="<p><a href='d.php'><strong>".$_SESSION['SesVar']['Dekan'][0]."</strong> (".$_SESSION['SesVar']['Dekan']
                        [1].")</a></p>";
                    break;
                case 3:
                    $preDiv.="<p><a href='p.php'><strong>".$_SESSION['SesVar']['Prepod'][0]."</strong> (".$_SESSION['SesVar']
                        ['Prepod'][1].")</a></p>";
                    break;
            }
        }
